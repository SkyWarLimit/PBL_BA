...
<body>
    ...
    <script>
        ...
        function loadBeranda() {
            ...
            Promise.all([
                fetch('api/booking.php').then(r => r.json()).catch(() => ({
                    data: []
                })), // Booking
                ...
                fetch('api/galeri.php').then(r => r.json()).catch(() => ({
                    data: []
                })), // Galeri (Baru ditambahkan untuk chart)
        ...
        function loadAnggota() {
            document.getElementById('page-title-text').innerText = 'Manajemen Dosen/Anggota';
            fetch('api/anggota.php').then(r => r.json()).then(result => {
                allAnggotaData = result.success ? result.data : [];
                const html = `
                    <div class="fade-in">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div class="search-input-wrapper w-50">
                                <i class="fas fa-search"></i>
                                <input type="text" id="searchAnggota" class="form-control search-input" placeholder="Cari nama atau NIDN anggota..." onkeyup="filterAnggota()">
                            </div>
                            <div class="d-flex gap-2">
                                <button class="btn-primary-custom rounded-pill px-4" onclick="openAnggotaForm()"><i class="fas fa-plus me-1"></i> Tambah Anggota</button>
                            </div>
                        </div>
                                    ...
                                    <table class="table table-modern align-middle">
                                        <thead class="bg-white">
                                            <tr>
                                                ...
                                                <th class="text-center" style="width: 15%;">Links</th>
        ...
        function renderAnggotaTable(data) {
            ...
            if (!data.length) { tbody.innerHTML = `<tr><td colspan="5" class="text-center py-5 text-muted"><div class="py-4"><i class="fas fa-user-tie fa-3x mb-3 text-light-emphasis"></i><p>Belum ada data anggota.</p></div></td></tr>`; return; }
            
            tbody.innerHTML = data.map(i => {
                const displayNama = i.nama_anggota || 'Nama Tidak Ditemukan';
                let displayImage = i.foto ? fixImagePath(i.foto) : `https://ui-avatars.com/api/?name=${displayNama}&background=4361ee&color=fff`;
                const linkCount = i.links ? i.links.length : 0;
                
                // Ambil 3 keahlian pertama dari array untuk tampilan ringkas
                const displayKeahlian = Array.isArray(i.bidang_keahlian) ? i.bidang_keahlian.slice(0, 3).join(', ') + (i.bidang_keahlian.length > 3 ? '...' : '') : '-';

                return `<tr>
                    <td class="ps-4">
                        <img src="${displayImage}" class="content-thumbnail" style="width: 55px; height: 55px; border-radius: 50%;" onerror="this.src='https://ui-avatars.com/api/?name=${displayNama}&background=4361ee&color=fff'">
                    </td>
                    <td>
                        <div class="content-title text-truncate">${displayNama}</div>
                        <small class="text-muted">NIDN: ${i.nidn}</small>
                    </td>
                    <td>
                        <span class="badge-modern badge-fasilitas">${displayKeahlian}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge-modern badge-default">${linkCount} Link(s)</span>
                    </td>
                    <td class="text-end pe-4">
                        <div class="btn-group">
                            <button class="btn btn-sm btn-light text-primary bg-white shadow-sm border me-1 rounded-2" onclick="openAnggotaForm(${i.id_dosen})" title="Edit"><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-sm btn-light text-danger bg-white shadow-sm border rounded-2" onclick="deleteAnggota(${i.id_dosen})" title="Hapus"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </td>
                </tr>`;
            }).join('');
        ...
        function deleteAnggota(id) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: "Data anggota ini akan dihapus permanen! (Termasuk semua data relasional)",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#e63946',
                cancelButtonColor: '#ced4da',
        ...
        function filterAnggota() { 
            ...
            const filtered = allAnggotaData.filter(i => {
                const nama = i.nama_anggota || '';
                // Sesuaikan pencarian karena keahlian sekarang adalah array
                const keahlian_str = Array.isArray(i.bidang_keahlian) ? i.bidang_keahlian.join(', ') : ''; 
                return nama.toLowerCase().includes(search) || i.nidn.toLowerCase().includes(search) || keahlian_str.toLowerCase().includes(search);
            });
        ...
        function openAnggotaForm(id = null) {
            if(id) {
                document.getElementById('page-title-text').innerText = 'Memuat Data...';
                fetch(`api/anggota.php?id=${id}`)
                    .then(r => r.json())
                    .then(res => renderAnggotaFormHTML(res.data))
                    .catch(() => Swal.fire('Error', 'Gagal memuat data anggota.', 'error'));
            } else {
                renderAnggotaFormHTML(null);
            }
        }

        // --- HELPER LINK AKADEMIK (DROPDOWN) ---

        function getLinkRowHtml(platform = '', url = '') {
            const platforms = [
            ...
            let platformOptions = platforms.map(p => 
                `<option value="${p.value}" ${platform === p.value ? 'selected' : ''}>${p.label}</option>`
            ).join('');
            platformOptions = `<option value="">-- Pilih Platform --</option>` + platformOptions;

            return `
                <div class="d-flex mb-2 align-items-center link-row">
                    <div class="flex-grow-1 me-2" style="max-width: 35%;">
                        <select name="link_platform[]" class="form-select" required>
                            ${platformOptions}
                        </select>
                    </div>
                    <div class="flex-grow-1 me-2">
                        <input type="url" name="link_url[]" class="form-control" placeholder="URL Lengkap" value="${url}" required>
                    </div>
                    <div class="flex-shrink-0" style="width: 40px;">
                        <button type="button" class="btn btn-outline-danger w-100" onclick="removeLinkRow(this)" title="Hapus Link"><i class="fas fa-trash-alt"></i></button>
                    </div>
                </div>`;
        }
        ...
        function addLinkRow() {
            ...
            container.insertAdjacentHTML('beforeend', getLinkRowHtml());
        }
        ...
        function searchUser(query) {
            ...
            fetch(`api/anggota.php?action=search_user&q=${query}`)
                .then(r => r.json())
                .then(res => {
                    resultsContainer.classList.add('shadow-sm');
                    resultsContainer.style.display = 'block';
                    if (res.success && res.data.length > 0) {
                        resultsContainer.innerHTML = res.data.map(user => `
                            <button type="button" class="list-group-item list-group-item-action py-2" onclick="selectUser(${user.id_user}, '${user.nama}', this)">
                                <h6 class="mb-0 fw-bold">${user.nama}</h6>
                                <small class="text-muted">${user.email}</small>
                            </button>
                        `).join('');
                    } else {
                        resultsContainer.innerHTML = `<div class="list-group-item text-muted small">User tidak ditemukan.</div>`;
                    }
                });
        }

        function selectUser(id, nama, button) {
            document.getElementById('id_user_terpilih').value = id;
            document.getElementById('input_nama_anggota').value = nama;
            
            // Visual konfirmasi
            document.getElementById('input_nama_anggota').classList.add('is-valid');
            
            // Sembunyikan hasil pencarian setelah memilih
            document.getElementById('user_search_results').style.display = 'none';
        }


        // --- HTML FORM ANGGOTA (TERMASUK SEARCH INPUT) ---

        function renderAnggotaFormHTML(data) {
            const isEdit = data !== null;
            const hasImage = isEdit && data.foto && data.foto !== '';
            const previewSrc = hasImage ? fixImagePath(data.foto) : '';
            
            const namaAnggotaVal = isEdit ? (data.nama_anggota || 'Administrator') : '';
            const idUserVal = isEdit ? (data.id_user_terpilih || '') : ''; 
            
            // Data relasional yang sudah di-join dan digabungkan menjadi string oleh API
            const pendidikanText = isEdit ? (data.pendidikan_terakhir || '') : ''; 
            const keahlianText = isEdit ? (data.bidang_keahlian || '') : ''; 
            
            document.getElementById('page-title-text').innerText = isEdit ? 'Edit Data Anggota' : 'Tambah Anggota Baru';

            let linkRows = '';
            if (isEdit && data.links && data.links.length > 0) {
                data.links.forEach(link => {
                    linkRows += getLinkRowHtml(link.platform, link.link_url);
                });
            } else {
                linkRows = getLinkRowHtml();
            }
            ...
            const html = `
                <div class="form-container-view fade-in">
                    <form id="formAnggota" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="old_foto" value="${isEdit ? (data.foto || '') : ''}">
                        <input type="hidden" name="id_user_terpilih" id="id_user_terpilih" value="${idUserVal}"> 

                        <div class="d-flex justify-content-between align-items-center mb-3">
                             <button type="button" class="btn btn-link text-decoration-none text-muted ps-0" onclick="loadAnggota()"><i class="fas fa-arrow-left me-2"></i>Kembali ke Daftar</button>
                             <button type="submit" class="btn btn-primary-custom px-5 rounded-pill"><i class="fas fa-save me-2"></i> ${isEdit ? 'Simpan Perubahan' : 'Tambah Anggota'}</button>
                        </div>
                        
                        <div class="row g-4">
                            
                            <!-- Kiri: Informasi Dasar (Lebih Lebar: col-lg-8) -->
                            <div class="col-lg-8">
                                <div class="card h-100">
                                    <div class="card-body">
                                        <h6 class="fw-bold text-primary mb-3 text-uppercase small ls-1">Informasi Dasar Anggota</h6>
                                        
                                        <div class="mb-3 position-relative">
                                            <label class="form-label fw-bold small text-muted">Nama User Dosen</label>
                                            
                                            <!-- Field Nama (Read-only di Edit, Search di Tambah) -->
                                            <input type="text" id="input_nama_anggota" class="form-control form-control-lg fw-bold" 
                                                value="${namaAnggotaVal || ''}" 
                                                ${isEdit ? 'disabled' : ''}
                                                placeholder="${isEdit ? namaAnggotaVal : 'Ketik untuk mencari nama di tabel User...'}" 
                                                onkeyup="${isEdit ? '' : 'searchUser(this.value)'}">

                                            <!-- Kontainer Hasil Pencarian -->
                                            <div id="user_search_results" class="list-group mt-1 shadow-sm" 
                                                 style="position: absolute; z-index: 100; max-height: 200px; overflow-y: auto; width: 100%; display: none;">
                                            </div>
                                            
                                            <div class="form-text text-muted small mt-1">
                                                <i class="fas fa-info-circle me-1"></i> Nama diikat ke user yang sudah terdaftar. ${isEdit ? '' : 'Pilih user dari hasil pencarian.'}
                                            </div>
                                        </div>
                                        
                                        <!-- Pendidikan Terakhir dan NIDN dipisah di dua kolom -->
                                        <div class="row g-3 mb-4">
                                            <div class="col-md-6">
                                                <label class="form-label fw-bold small text-muted">NIDN</label>
                                                <input type="text" name="nidn" class="form-control" value="${isEdit ? data.nidn : ''}" required placeholder="NIDN Anggota">
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label fw-bold small text-muted">Pendidikan Terakhir (Contoh: S3, S2, S1)</label>
                                                <textarea name="pendidikan_terakhir" class="form-control" rows="1" required placeholder="Contoh: S3 Ilmu Komputer, S2, S1">${pendidikanText}</textarea>
                                                <div class="form-text text-muted small"><i class="fas fa-info-circle me-1"></i> Dipisahkan dengan koma (,) atau baris baru.</div>
                                            </div>
                                        </div>
                                        
                                        <div class="mb-0">
                                            <label class="form-label fw-bold small text-muted">Bidang Keahlian</label>
                                            <textarea name="bidang_keahlian" class="form-control" rows="8" required placeholder="Tuliskan bidang keahlian (dipisahkan koma, e.g. Data Science, NLP, Machine Learning)">${keahlianText}</textarea>
                                            <div class="form-text text-muted small"><i class="fas fa-info-circle me-1"></i> Dipisahkan dengan koma (,) atau baris baru.</div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Kanan: Foto dan Link (col-lg-4) -->
                            <div class="col-lg-4">
                                
                                <!-- Foto Profil -->
                                <div class="card mb-4">
                                    <div class="card-body">
                                        <h6 class="fw-bold text-primary mb-3 text-uppercase small ls-1">Foto Profil</h6>
                                        <div class="upload-container">
                                            <div id="uploadPlaceholder" class="upload-box ${hasImage ? 'd-none' : ''}" style="height: 200px; border-radius: 50%;">
                                                <input type="file" name="foto" id="fileInput" class="file-input-hidden" accept="image/*" onchange="previewFile(this)">
                                                <div class="upload-icon-circle" style="width:40px; height:40px;"><i class="fas fa-camera fs-5"></i></div>
                                                <div class="upload-text-title small">Upload Foto</div>
                                                <div class="upload-text-muted small">JPG/PNG Max 2MB</div>
                                            </div>
                                            <div id="imagePreviewContainer" class="preview-box ${hasImage ? '' : 'd-none'}" style="height: 200px; padding: 0;">
                                                <img src="${previewSrc}" id="previewImg" alt="Preview" style="height: 100%; width: 100%; border-radius: 50%; object-fit: cover;">
                                                <button type="button" class="btn-remove-preview" onclick="resetUpload(true)" title="Ganti Foto" style="top: 10px; right: 10px;"><i class="fas fa-sync-alt"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="fw-bold text-primary mb-3 text-uppercase small ls-1">Link Akademik</h6>
                                        <div id="linkContainer">
                                            ${linkRows}
                                        </div>
                                        <button type="button" class="btn btn-outline-primary btn-sm w-100" onclick="addLinkRow()"><i class="fas fa-plus me-1"></i> Tambah Link</button>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            document.getElementById('content-area').innerHTML = html;
            document.getElementById('formAnggota').onsubmit = (e) => { e.preventDefault(); submitFormPage('api/anggota.php', new FormData(e.target), loadAnggota); };
        }
    </script>
</body>
